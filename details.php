...
// Set the locale for PHP currency formatting
$php_currency_symbol = '₱';
...
<div class="item active">
    <center>
        <img src="admin_area/product_images/<?php echo $pro_img1; ?>" class="img-responsive" style="width: 550px; height: 550px;">
    </center>
</div>
...
<div class="item">
<center>
<img src="admin_area/product_images/<?php echo $pro_img2; ?>" class="img-responsive" style="width: 550px; height: 550px;">
</center>
</div>

<div class="item">
<center>
<img src="admin_area/product_images/<?php echo $pro_img3; ?>" class="img-responsive" style="width: 550px; height: 550px;">
</center>
</div>
...
if (isset($_POST['add_cart'])) {
    ...
    $product_size = isset($_POST['product_size']) ? $_POST['product_size'] : 'DEFAULT'; // Default to 'DEFAULT' if not set

    // Check for duplicate entry before inserting
  // Check for duplicate entry before inserting
$check_duplicate = "SELECT * FROM cart WHERE ip_add='$ip_add' AND p_id='$p_id' AND size='$product_size'";
$run_duplicate_check = mysqli_query($con, $check_duplicate);

if (mysqli_num_rows($run_duplicate_check) > 0) {
    ...
    if ($run_price && $row_price = mysqli_fetch_array($run_price)) {
        $pro_price = $row_price['product_price'];
        $pro_psp_price = $row_price['product_psp_price'];

        // Adjust product price based on label
        ...
        if ($run_query) {
            ...
            echo "<script>alert('Error inserting into cart')</script>";
        }
    }
}
...
<label class="col-md-5 control-label" >Nic / MG</label>
...
<label class="col-md-5 control-label" >Bundle Nic / MG</label>

<div class="col-md-7" ><!-- col-md-7 Starts -->

<select name="product_size" class="form-control" >

<option>Select a MG</option>
<option>DEFAULT</option>
<option>0MG</option>
<option>3MG</option>
<option>6MG</option>
...
if($status == "product"){
  ...
  if ($pro_label == "Sale" or $pro_label == "Gift") {
    echo "
    <p class='price'>
        Product Price : <del> $php_currency_symbol" . number_format($pro_price, 2) . " </del><br>
        Product Sale Price : $php_currency_symbol" . number_format($pro_psp_price, 2) . "
    </p>";
} else {
    echo "
    <p class='price'>
        Product Price : $php_currency_symbol" . number_format($pro_price, 2) . "
    </p>";
}
  ...
  if ($pro_label == "Sale" or $pro_label == "Gift") {
    echo "
    <p class='price'>
        Bundle Price : <del> $php_currency_symbol" . number_format($pro_price, 2) . " </del><br>
        Bundle Sale Price : $php_currency_symbol" . number_format($pro_psp_price, 2) . "
    </p>";
} else {
    echo "
    <p class='price'>
        Bundle Price : $php_currency_symbol" . number_format($pro_price, 2) . "
    </p>";
}
...
<img src="admin_area/product_images/<?php echo $pro_img1; ?>" class="img-responsive" style="width: 170px; height: 165px;">
...
<img src="admin_area/product_images/<?php echo $pro_img2; ?>" class="img-responsive" style="width:  170px; height: 165px;">
...
<img src="admin_area/product_images/<?php echo $pro_img3; ?>" class="img-responsive" style="width:  170px; height: 165px;">
...
while ($row_products = mysqli_fetch_array($run_products)) {
    ...
    // Format Prices with PHP Currency Symbol
    if ($pro_label == "Sale" or $pro_label == "Gift") {
        $product_price = "<del> $php_currency_symbol" . number_format($pro_price, 2) . " </del>";
        $product_psp_price = "| $php_currency_symbol" . number_format($pro_psp_price, 2);
    } else {
        $product_price = "$php_currency_symbol" . number_format($pro_price, 2);
        $product_psp_price = "";
    }
